var HelloWorldLayer = cc.Layer.extend({
    ctor:function () {
        ...
        self.wxData = {
            "appId": "", // 服务号可以填写appId
            "imgUrl" : 'http://www.6m5m.com',
            "link" : 'http://www.6m5m.com',
        ...
        WeixinApi.ready(function(Api) {
            ...
            var wxCallbacks = {
                ...
                confirm : function(resp) {
                    ...
                    //window.location.href='http://www.6m5m.com/';
                    // alert("分享成功");
                },
                // 整个分享过程结束
                all : function(resp) {
                    ...
                    // alert("分享结束");
                    if(self.isGameOver){
                        if(window.location) window.location.href = "http://www.6m5m.com/";
                    }
                }
            };
    ...
    gameOver: function(){
        ...
        if(self.score <= 0){
            self.wxData.desc = '蹲了'+self.hour+'个多小时，腿都麻了，我连半条若鹭都木有调到';
        }else{
            self.wxData.desc = '经过'+self.hour+'个多小时的蹲点，我成功的钓到了'+self.score+'条若鹭';
        }
        var text = self.wxData.desc;

        layerMask.delay(0.5).then(function(){
            layerMask.attr('opacity', 128);
            var share = cc.createSprite('http://www.6m5m.com', {
                anchor: [1.0, 1.0],
                xy: [720, 1280],
                opacity: 0,
                scale: 0.5
